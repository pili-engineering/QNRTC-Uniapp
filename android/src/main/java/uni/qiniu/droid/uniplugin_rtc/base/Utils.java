...
public class Utils {
    public static QNRTCSetting toRTCSetting(JSONObject configuration) {
        ...
        String scene = configuration.getString(Constants.AUDIO_SCENE);
        QNAudioScene audioScene = QNAudioScene.DEFAULT;
        switch (scene) {
            case "VOICE_CHAT":
        ...
        boolean defaultAudioRouteToSpeakerphone = configuration.getBooleanValue(Constants.DEFAULT_AUDIO_ROUTE_TO_SPEAKERPHONE);
        int reconnectionTimeout = configuration.getIntValue(Constants.RECONNECTION_TIMEOUT);
        ...
        QNRTCSetting setting = new QNRTCSetting()
                ...
                .setDefaultAudioRouteToSpeakerphone(defaultAudioRouteToSpeakerphone)
    ...
    public static List<UniLocalVideoTrackStats> convertToUniLocalVideoTrackStats(List<QNLocalVideoTrackStats> videoTrackStats) {
        ...
        for (QNLocalVideoTrackStats stats : videoTrackStats) {
            UniLocalVideoTrackStats localVideoTrackStats = new UniLocalVideoTrackStats(
                    stats.profile.name(), stats.uplinkFrameRate, stats.uplinkBitrate, stats.uplinkRTT, stats.uplinkLostRate, stats.uplinkFrameWidth, stats.uplinkFrameHeight, stats.captureFrameRate, stats.captureFrameWidth, stats.captureFrameHeight, stats.targetFrameRate, stats.targetFrameWidth, stats.targetFrameHeight);
...
//        transcodingLiveStreamingConfig.setHoldLastFrame(config.getBoolean(Constants.KEY_HOLD_LAST_FRAME));
    ...
    public static QNAudioScene toAudioScene(String type) {
        QNAudioScene audioScene;
        if ("DEFAULT".equals(type)) {
            audioScene = QNAudioScene.DEFAULT;
        } else if ("VOICE_CHAT".equals(type)) {
            audioScene = QNAudioScene.VOICE_CHAT;
        } else {
            audioScene = QNAudioScene.SOUND_EQUALIZE;
        }
    ...
    public static QNClientRole toClientRole(String type) {
        QNClientRole clientRole;
        if ("BROADCASTER".equals(type)) {
            clientRole = QNClientRole.BROADCASTER;
        } else {
            clientRole = QNClientRole.AUDIENCE;
        }
        return clientRole;
    }

    public static QNMediaRelayConfiguration toMediaRelaySetting(JSONObject configuration) {
        String roomName = configuration.getString(Constants.ROOM_NAME);
        String relayToken = configuration.getString(Constants.RELAY_TOKEN);
        QNMediaRelayInfo mediaRelayInfo = new QNMediaRelayInfo(roomName, relayToken);
        QNMediaRelayConfiguration relayConfiguration = new QNMediaRelayConfiguration(mediaRelayInfo);
        JSONArray desRoomInfo = configuration.getJSONArray(Constants.DES_ROOM_INFO);
        for (int i = 0; i < desRoomInfo.size(); i++) {
            JSONObject info = desRoomInfo.getJSONObject(i);
            String desRoomName = info.getString(Constants.ROOM_NAME);
            String desRelayToken = info.getString(Constants.RELAY_TOKEN);
            QNMediaRelayInfo desMediaRelayInfo = new QNMediaRelayInfo(desRoomName, desRelayToken);
            relayConfiguration.addDestRoomInfo(desMediaRelayInfo);
        }
        return relayConfiguration;
    }

    public static QNRTCClientConfig toClientConfig(JSONObject configuration) {
        QNClientMode mode = configuration.getString(Constants.CLIENT_MODE).equals("RTC") ? QNClientMode.RTC : QNClientMode.LIVE;
        QNClientRole role = configuration.getString(Constants.CLIENT_ROLE).equals("BROADCASTER") ? QNClientRole.BROADCASTER : QNClientRole.AUDIENCE;
        QNRTCClientConfig config = new QNRTCClientConfig(mode, role);
        return config;
    }
}
