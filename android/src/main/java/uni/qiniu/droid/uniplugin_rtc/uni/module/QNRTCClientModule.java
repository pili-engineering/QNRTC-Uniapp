...
public class QNRTCClientModule extends UniModule implements QNRTCGlobalListener {
    ...
    @UniJSMethod(uiThread = false)
    public void setClientRole(String clientRole, UniJSCallback callback) {
        getManager().setClientRole(Utils.toClientRole(clientRole), new UniCallback(callback));
    }

    @UniJSMethod(uiThread = false)
    public void startMediaRelay(JSONObject config, UniJSCallback callback) {
        getManager().startMediaRelay(Utils.toMediaRelaySetting(config), new UniCallback(callback));
    }

    @UniJSMethod(uiThread = false)
    public void updateMediaRelay(JSONObject config, UniJSCallback callback) {
        getManager().updateMediaRelay(Utils.toMediaRelaySetting(config), new UniCallback(callback));
    }

    @UniJSMethod(uiThread = false)
    public void stopMediaRelay(UniJSCallback callback) {
        getManager().stopMediaRelay(new UniCallback(callback));
    }
    ...
    public JSONObject getRemoteVideoTrackStats() {
        ...
        for (Map.Entry<String, QNRemoteVideoTrackStats> entry : videoTrackStats.entrySet()) {
            UniRemoteVideoTrackStats remoteVideoTrackStats = new UniRemoteVideoTrackStats(
                    ...
                    entry.getValue().uplinkLostRate, entry.getValue().uplinkFrameWidth, entry.getValue().uplinkFrameHeight);
